...
class pos_close_statement(osv.TransientModel):
    _inherit = 'pos.close.statement'
    ...
    def close_statement(self, cr, uid, ids, context=None):
        ...
        ids = statement_obj.search(cr, uid, [('state', '!=', 'confirm'),
         ('journal_id', 'in', j_ids)], context=context)
        if not ids:
        ...
        for statement in statement_obj.browse(cr, uid, ids, context=context):
            ...
            if not statement.journal_id.check_dtls:
                statement_obj.button_confirm_cash(cr, uid, [statement.id],
                 context=context)
