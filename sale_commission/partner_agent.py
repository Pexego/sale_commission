...
"""
    objeto de comportamiento many2many que relaciona agentes o comerciales con
    partners
"""
...
class res_partner_agent(orm.Model):
    ...
    def _get_partner_agents_to_update_from_sale_agents(self, cr, uid, ids,
                                                       context=None):
        """
            devuelve los ids de partner agents a actualizar desde el
            lanzamiento de un evento de actualización en agentes de ventas
        """
        if context is None:
            context = {}
        result = []
        for agent_obj_id in self.browse(cr, uid, ids):
            partner_agents_ids = self.pool.get('res.partner.agent').search(
                cr, uid, [('agent_id', '=', agent_obj_id.id)])
    ...
    _columns = {
        ...
        'type': fields.related('agent_id', 'type', type="selection",
                               selection=[('asesor', 'Adviser'),
                                          ('comercial', 'Commercial')],
                               readonly=True,
                               store={
                                   'sale.agent':
                                       (_get_partner_agents_to_update_from_sale_agents,
                                        ['type'], 10),
                                   'res.partner.agent':
